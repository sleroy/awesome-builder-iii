<template>
  <div>
    ...
    <div class="flex w-full items-center justify-center mt-6">
      <button class="flex justify-center items-center select-none bg-primary hover:p-6 border-2 text-white text-xl font-bold p-4 m-2 rounded-full shadow focus:outline-none focus:shadow-outline" @click="upload" :disabled="state.progress || (state.accepted.length == 0)">        
          <svg aria-hidden="true" class="w-7 h-7 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-red-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg"  v-if="state.progress">
          ...
          <span class="">{{ state.progress ?   "Upload in progress"  : "Upload selected files" }}</span>      
      </button>
    </div>
  </div>
</template>
